...
\caption{Curva ROC del clasificador SVM en datos originales (fold 1). El AUC macro de 0.9933 representa un alto rendimiento, evidenciando la capacidad del kernel RBF para capturar fronteras de decisión complejas.}
\label{fig:roc_svm}
\end{figure}

El clasificador SVM (Figura~\ref{fig:roc_svm}) obtiene un AUC macro de 0.9933 en el primer pliegue. Este resultado se debe a la capacidad del kernel de función de base radial para proyectar los datos a un espacio de características de alta dimensionalidad donde las clases se vuelven linealmente separables.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{RF_original_1.png}
\caption{Curva ROC del clasificador Random Forest en datos originales (fold 1). El AUC macro de 0.9967 refleja la robustez del método ensemble basado en árboles, alcanzando el mayor AUC entre los modelos individuales en este pliegue.}
\label{fig:roc_rf}
\end{figure}

Random Forest (Figura~\ref{fig:roc_rf}) presenta un AUC macro de 0.9967, el más alto entre los modelos individuales en este pliegue específico. Este resultado refleja la capacidad del método ensemble basado en árboles para capturar fronteras de decisión complejas mediante la agregación de múltiples árboles de decisión.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{NB_original_1.png}
\caption{Curva ROC del clasificador Naive Bayes en datos originales (fold 1). A pesar de la suposición simplificadora de independencia entre características, el método alcanza un AUC macro de 0.9833, demostrando su efectividad en este dominio.}
\label{fig:roc_nb}
\end{figure}

Naive Bayes (Figura~\ref{fig:roc_nb}) alcanza un AUC macro de 0.9833, resultado notable considerando la suposición simplificadora de independencia condicional entre características que difícilmente se cumple en datos biométricos reales. Las curvas muestran formas escalonadas características de clasificadores que operan con estimaciones de probabilidad discretizadas, pero el rendimiento global es comparable a métodos más complejos.
...
El ensemble por mediana de probabilidades utiliza la mediana en lugar del promedio para agregar las probabilidades de los clasificadores base. Teóricamente, la mediana proporciona mayor robustez frente a predicciones extremas de modelos individuales que puedan estar mal calibrados: un único modelo con predicción extremadamente confiada (probabilidad cercana a 1.0) o extremadamente dudosa (probabilidad cercana a 0.33 en un problema de tres clases) tiene menor influencia en el resultado final. En los datos analizados, los tres métodos de ensemble muestran rendimientos comparables, lo cual es esperable cuando los clasificadores base están relativamente bien calibrados y producen predicciones consistentes.
...
KNN en datos originales no solo alcanza el mejor F1-score medio (0.9799), sino que también exhibe la menor desviación estándar (0.0183), inferior a la desviación de Random Forest (0.0238) en la misma configuración. Esta baja variabilidad indica que el rendimiento de KNN es robusto a la composición específica del conjunto de entrenamiento: independientemente de qué 80\% de las instancias se utilicen para entrenar y qué 20\% se reserven para test en cada pliegue de validación cruzada, el modelo alcanza rendimiento consistentemente alto. Esta propiedad es crucial en aplicaciones prácticas donde el conjunto de entrenamiento disponible puede ser una muestra sesgada de la población real, y donde las predicciones deben ser fiables incluso cuando los datos futuros presenten características ligeramente distintas.
...
Los métodos de ensemble, aunque no proporcionan mejoras dramáticas en rendimiento medio en este problema relativamente sencillo donde los mejores modelos individuales ya alcanzan rendimientos cercanos al óptimo, ofrecen una estrategia práctica para combinar múltiples clasificadores. La convergencia de los tres métodos de ensemble (votación, media y mediana) hacia rendimientos similares en este problema específico sugiere que, cuando los clasificadores base están bien calibrados y producen predicciones consistentes, la estrategia particular de combinación tiene impacto marginal.
